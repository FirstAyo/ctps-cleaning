...
## Phase 11.3 Blog writing workspace

Blog post create/edit routes use a document-first workspace inside the existing Admin shell. The global header remains sticky at `4rem`; a separate publishing command bar sticks beneath it, and a separate formatting toolbar sticks beneath the publishing bar. Shared CSS height tokens also place the desktop settings sidebar and content focus/scroll margins below the complete stack. Dialogs layer above both bars, and toolbar overflow is horizontal and contained at narrower widths rather than causing page overflow.

The publishing row contains navigation, lifecycle status, explicit save state, word count, reading time, Settings, Preview, Save, Schedule, review, publish, and unpublish actions as permitted. The formatting row exposes Paragraph/H2/H3/H4, bold, italic, underline, lists, blockquote, safe link, Blog image, divider, undo, and redo. Controls have labels, tooltips, focus/disabled states, and selection-derived active states. The writing canvas contains a large labeled title, nearby excerpt and count, and article typography; H1 remains reserved for the post title.

Desktop shows the grouped settings sidebar. Below the desktop breakpoint, the same controlled settings open in a modal sheet and both command rows stay available. Settings cover publishing, featured Blog media, category/tags/author constraints, slug and existing per-post SEO fields, a search preview, publishing checks, revisions, and existing archive/delete actions. Phase 11.3 does not add site-wide SEO tooling.

Saves are explicit rather than per-keystroke autosaves. Dirty/saving/saved/error states are announced, publish/schedule require a saved version, failed or conflicting saves retain local editor state, and browser/back navigation warns about unsaved work. Revision restore creates a new revision through optimistic concurrency rather than mutating history.
